...
function setupChoiceButtons() {
    ...
    if (!yesBtn || !noBtn || !question || !overlay) return;

    yesBtn.addEventListener("click", () => {
        question.textContent = "Teşekkür ederim.";
        question.classList.remove("warning");
        question.classList.add("accepted");

        yesBtn.style.display = "none";
        noBtn.style.display = "none";

        overlay.classList.add("light-ending");
    });

    noBtn.addEventListener("click", () => {
        noClickCount++;

        noBtn.classList.add("disturbed");
        ...
        if (noClickCount === 1) {
            ...
            question.textContent = "Duydum.";
            noBtn.textContent = "yine hayır";
        } else if (noClickCount === 3) {
            question.textContent = "Ama hâlâ buradasın.";
            noBtn.textContent = "hayır";
            noBtn.classList.add("shrink");
            yesBtn.classList.add("inviting");
        } else {
            question.textContent = "Seçmemek de bir seçim.";
            noBtn.classList.add("disabled-feeling");
            yesBtn.textContent = "yardım et";
        }

        setTimeout(() => {
            noBtn.classList.remove("disturbed");
        }, 700);
    });
}
